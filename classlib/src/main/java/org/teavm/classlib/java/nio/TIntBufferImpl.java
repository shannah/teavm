...
abstract class TIntBufferImpl extends TIntBuffer {
    ...
    @Override
    public TIntBuffer put(int index, int b) {
        if (isReadOnly()) {
            throw new TReadOnlyBufferException();
        }
        if (index < 0 || index >= limit) {
