...
public class BigIntegerModPowTest {
    ...
    public void testModPowZeroExp() {
        ...
        for (BigInteger aBase : base) {
            for (int j = 0; j < mod.length; ++j) {
                assertEquals(aBase + " modePow(" + exp + ", " + mod[j] + ") should be " + BigInteger.ONE,
                        BigInteger.ONE, aBase.modPow(exp, mod[j]));
            }
        }
        ...
        for (BigInteger aBase : base) {
            for (int j = 0; j < mod.length; ++j) {
                assertEquals(aBase + " modePow(" + exp + ", " + mod[j] + ") should be " + BigInteger.ZERO,
                        BigInteger.ZERO, aBase.modPow(exp, mod[j]));
            }
        }
    }
